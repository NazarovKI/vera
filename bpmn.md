...
tags:
  ...
  - амо
published: true
goal: Описание бизнес-процессов компании
---
# Бизнес как процесс
...
# [процессы в компании](2025-12-03-order.md) Какие  существуют  и отношения между ними?

[Квалификация лида](scoring.md) > [Выявление потребности](выявление%20потребности.md) > [Предварительный расчёт](raschot.md) > [Проведение замера](регламент%20замера.md) > [Оформление догововора](оформление%20договора.md) > Оплата > Монтаж > Рекламация > Сервис

Процессы объединяет их принадлежность к сущности "сделка", 
каждый процесс имеет свои стадии. 
Воронка создаётся только для процессов в которых требуется оплата, где участвуют деньги. 
Остальные должны быть сделаны как смартпроцессы.
...
> Сделка создаётся только на стадии "Замер назначен". На вопрос "что сделать?" отвечает задача, в заголовке - тема (проект). Что сделать?

```mermaid
   kanban
column1[Замер назначен]
    task1[Задача: Провести замер дата адрес имя телефон. Указать ссылку на заказ. Ответственный: замерщик]
    task2[Задача: Прикрепить рисунок с замера. Если клиент только интересовался ценой отметить договор не заключен. Ответственный: замерщик]
column2[Замер проведен] 
    task1[Задача: Подготовить и отправить КП. КП должно отражать окончательную стоимость согласованную с инженером. Ответственный: менеджер]
    task2[Задача: Получить обратную связь по КП. Ответственный: менеджер]
    task3[Задача: Подписать договор с Заказчиком. Ответственный: менеджер]
column3[Договор подписан]
    task1[Задача: Контроль получения оплаты, внесение информации о промежуточном платеже в CRM, передать заказ в производство. Ответственный: бухгалтер]
    task2[Задача: Контроль получения заработной платы, на этом этапе сделка считается успешной
Ответственный: менеджер]
column4[Заказ передан в производство]
    task1[Обязательное поле: Номер заказа. Связать с дилерской программой. Ответственный: прораб]
    task2[При получении уведомления о готовности. Задача: Проверить получение оставшейся части оплаты. Ответственный: менеджер]
column5[Заказ готов]    
    task1[Задача: Позвонить клиенту. Запросить у бухгалтера подтверждение получения оплаты прорабу. Ответственный: менеджер]
     task2[Задача: Подтверидить оплату прорабу. Ответственный: бухгалтер]
    task3[Задача: Назначить дату монтажа. Ответственный: прораб]
column6[Монтаж назначен]
```

Не должно быть сделок без задач. CRM должна поставить любую задачу
...
*Этап «Плачу»* завершает сделку, если все три предыдущих этапа клиент успешно прошёл. Ударение на последний слог
...
# Бизнес-процессы:
Умные сценарии нужны для расчёта, замера, договора. 
Это позволит посчитать конверсию и понять на каком этапе провалы. 
Позволит автоматизировать [расчёт заработной платы](financial.md), 
которая включает расчёт, замер, договор. 
Каждый из процессов должен иметь обязательные поля, 
лучше ими ограничиться. 
Должен создавать событие в ленте. 
Чтобы другие видели работу. 
Появился расчёт - замерщики уже приготовились решается кто поедет.
# Расчёт КП (подготовка предложения) 
